...
public class FillEffect {
  ...
  public FillEffect create() {
     FillEffect value = new FillEffect();

     if( bounds != null ) {
       value.bounds = new Rectangle2D.Double( bounds.getX(), bounds.getY(), bounds.getWidth(), bounds.getHeight() );
     }

     value.halfTransparent = this.halfTransparent;

     if( this.firstGradientColor != null ) {
        value.firstGradientColor = new Color( this.firstGradientColor.getRGB() );
     }
     if( this.secondGradientColor != null ) {
        value.secondGradientColor = new Color( this.secondGradientColor.getRGB() );
     }
  ...
  public void setTexturePaint(BufferedImage image) {
    this.image = image;
    if( image != null ) {
       this.halfTransparent = false;
       this.firstGradientColor = null;
  ...
  public Paint getGradientPaint() {
    ...
    if( firstGradientColor != null && secondGradientColor != null && bounds != null ) {

      Point2D fromPoint = this.getFromPoint();
      Point2D toPoint = this.getToPoint();
      int type = this.type;
      if( type == ROUND ) {
         return new RoundGradientPaint( this, fromPoint, firstGradientColor, toPoint, secondGradientColor );
      } else if( type == FROM_CENTER ) {
         return new RectangleGradientPaint( this, fromPoint, firstGradientColor, toPoint, secondGradientColor );
      } else if( type == FROM_CORNER ) {
         return new CornerGradientPaint( this, fromPoint, firstGradientColor, toPoint, secondGradientColor);
      } else {
         return new GradientPaint( fromPoint, firstGradientColor, toPoint, secondGradientColor, cyclic);
      }
    } else {
      return null;
    }
  ...
  public TexturePaint getHalfTransparentTexturePaint(Color c) {
     ...
     Utility.debug( this, "Getting Half Transparent Texture Paint ......" );

     // 기준 버퍼드 이미지 생성
     BufferedImage refBImage = Utility.getResourceBufferedImage( "half_transparent.gif" );
     ...
     int srcPixel, dstPixel,
         blackPixel = Color.black.getRGB(),
         firstTileColorPixel = c.getRGB();
  ...
  public TexturePaint getTexturePaint() {
    ...
    if( this.image == null ) {
      ...
      Frame f = new Frame();
      f.addNotify();
      Image image = new javax.swing.ImageIcon(this.image).getImage();
      ...
      return new TexturePaint( bImage, tr );
    }
  }
