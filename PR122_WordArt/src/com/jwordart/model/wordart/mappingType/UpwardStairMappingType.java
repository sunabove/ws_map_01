...
public class UpwardStairMappingType extends NonLinearMappingType {
  float w; // 폭 값
  float h; // 높이 값
  float cpY0, cpY1;// 아랫 및 위 콘트롤 포인트 수직 값
  float a0, a1;  // 아랫 기울기, 윗 기울기

  public UpwardStairMappingType(int type) {
     super(type);
  }
...
public float [] f(float [] point) {
    ...
    float refY0 = a0*x + h, refY1 = a1*x + cpY1;
...
public void setControlPoint(WordArt wa) {
     wa.setControlPoint( new ControlPointVertical(wa, 0.3, 1, 0.3, ControlPointVertical.RIGHT) );
  }
}