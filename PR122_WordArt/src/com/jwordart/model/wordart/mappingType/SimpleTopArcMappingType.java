 ...
 * 각각의 글립의 회전 각도만 달라진다.
 */
public class SimpleTopArcMappingType extends WordArtMappingType
     ...
     *             현 매핑에서는 중요한 의미는 없다.
     */
    public SimpleTopArcMappingType(int type) {
      super(type);
    }
    ...
    public AffineTransform getTransformInstance(WordArt wa, Shape glyph) {
       ...
       theta = FunctionUtil.getThetaOfArcLength(a, b, theta, cx*arcLength/(2.0*a*scaleX), true);
       ...
       AffineTransform at = AffineTransform.getTranslateInstance( ax + a, ay + b); // 이동
       at.rotate( theta, 0, 0 );    // 회전
       at.translate( - cx, - cy );  // 이동
       at.scale( scaleX, scaleY);   // 스케일
