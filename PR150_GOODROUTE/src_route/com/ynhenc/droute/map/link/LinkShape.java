...
public class LinkShape extends ComLibRoute {
	...
	private double [] loadShapeFromWkb() throws Exception {
		...
		if(qr.hasNext()) {
			...
			byte byteOrder = in.readByte(); // one byte
			int wkbType = in.readInt(); // four byte
			int numPoints = in.readInt(); // four byte
	...
	public static void main(String[] args) throws Exception {
		...
		if( true ) {
			...
			//System.out.println( linkShape.toKmlCoords() );
		}
		...
		if( true ) {
			...
			File file = new File( "C:\\Documents and Settings\\sbmoon\\¹ÙÅÁ È­¸é\\kml\\linkShape.kml");
