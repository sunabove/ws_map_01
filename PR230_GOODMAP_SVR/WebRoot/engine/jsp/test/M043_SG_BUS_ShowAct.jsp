...
<c:set var="sql" scope="request" >
   SELECT T1.MAP_X AS X_POS,T1.MAP_Y AS Y_POS,count(*) over() as CNT
       ...
       '#FFCC00' AS POI_BGCOLOR , '#3322CC' AS POI_LINECOLOR , 2 AS POI_LINEWIDTH ,
       -- T1.PLATE_NO  AS TOOLTIPCONTENT,
       --'<font color="red">' || T1.PLATE_NO || '</font>' AS TOOLTIPCONTENT,
       --'<a href="http://www.yahoo.com">' || T1.PLATE_NO || '</a>' AS TOOLTIPCONTENT,
       '<a onclick="alert(''' || T1.PLATE_NO || ''')">' || T1.PLATE_NO || '</a>' AS TOOLTIPCONTENT,
       --'aaa<script>alert( "def" )</script>' AS TOOLTIPCONTENT, 
       'M043_SG_BUS_ShowAct' AS LAYER_NAME , 3 AS LAYER_NO 
   FROM (     
...
<jsp:include flush="true" page="../M013_Nosun_ShowAct.jsp">
    ...
    <jsp:param name="layerName" value="bus_stop_001___" />
