...
function getGisEngine_MapUrlHead() {
    return "<%= basePath + "engine/jsp/" %>" ;
}

function getWebServer_ProxyServerPage() {
	return "";
    return "<%= basePath + "engine/jsp/M014_ProxyPage_WebSvrToMapSvr.jsp" %>" ;
}
...
function callAjax( url , onSuccessCall ) {
    ...
    var proxyServerPage = getWebServer_ProxyServerPage() ;  // ProxyServerPage
    var urlRedirect = getGisEngine_MapUrlHead() + url ;

    if( false ) { 
	    alert( "proxyServerPage=" + proxyServerPage );
	    alert( "urlRedirect=" + urlRedirect );
    }
    
    if( proxyServerPage == null || proxyServerPage == "" ) {
        // in case of no proxy
        proxyServerPage = url ;
        urlRedirect = "" ;
    } 
    ...
    callAjaxUrlParams( proxyServerPage , parameters, onSuccessCall ); 
}
