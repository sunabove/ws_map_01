...
public class TopCylinderMappingType extends NonLinearMappingType {
  float mw; // 폭 중간 값
  float h;  // 높이 값
  ...
  public TopCylinderMappingType(int type) {
     super(type);
  }
...
public float [] f(float [] point) {
    ...
    float refY = a*Math.abs((x - mw)*(x - mw)*(x - mw));
...
public void setControlPoint(WordArt wa) {
     wa.setControlPoint( new ControlPointVertical(wa, 0.65, 1.0, 0.8, ControlPointVertical.CENTER) );
  }
}