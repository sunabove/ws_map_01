...
public class BottomCurveMappingType extends NonLinearMappingType {
  float w;  // 폭 중간 값
  float h;  // 높이 값
  float a, b;  // y = a*x*x*x + b

  public BottomCurveMappingType(int type) {
     super(type);
  }
...
public float [] f(float [] point) {
    ...
    double refY0 = a*Math.abs((x - 0.8*w)*(x - 0.8*w)*(x - 0.8*w)) + b;
...
public void setParameters(WordArt wa) {
    ...
    a = cpY/(0.2F*0.2F*0.2F - 0.8F*0.8F*0.8F)/w/w/w;
    b = -a*0.8F*0.8F*0.8F*w*w*w;
  }

  @Override
public void setControlPoint(WordArt wa) {
     wa.setControlPoint( new ControlPointVertical(wa, 0.0, 0.6, 0.45, ControlPointVertical.RIGHT) );
  }
}