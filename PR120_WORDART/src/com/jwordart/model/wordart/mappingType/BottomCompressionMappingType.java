...
public class BottomCompressionMappingType extends NonLinearMappingType {
  float mw; // 폭 중간 값
  float h;  // 높이 값
  float cpY; // 콘트롤 포인트 수직 값
  float a;  // y = a*x*x

  public BottomCompressionMappingType(int type) {
     super(type);
  }
...
public void setControlPoint(WordArt wa) {
     wa.setControlPoint( new ControlPointVertical(wa, 0.05, 1.0, 0.6, ControlPointVertical.CENTER) );
  }
}