...
public class TainySqlCmd {
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		BufferedReader startReader = null;
		String[] fields;
		Connection con;
		Statement stmt;
		FieldTokenizer ft;
		PreparedStatement pstmt = null;
		int i, rsColCount, endAt, colWidth, colScale, colPrecision, typeCount, colType, parameterIndex, b1, b2, parameterInt, startAt, columnIndex, valueIndex;
		String tableName = null, inputString, cmdString, colTypeName, dbType, parameterString, loadString, fieldString, readString;
		...
		try {
			...
			com.sqlmagic.tinysql.dbfFileDriver a;
		...
		// String dbfDir = "C:/home/gis/ygis_map/";
		...
		System.out.println("===================================================");
		System.out.println(dbType + " Command line interface version " + dbVersion + " released March 15, 2007");
		...
		while (!cmdString.toUpperCase().equals("EXIT")) {
			try {
				if (startReader != null) {
					 ...
					 * commands broken over several lines. However, they cannot
					 * have partial commands on a line.
					 */
				...
				if (inputString.toUpperCase().startsWith("EXIT") | inputString.toUpperCase().startsWith("QUIT")) {
					break;
				}
				startAt = 0;
				while (startAt < inputString.length() - 1) {
					...
					if (cmdString.toUpperCase().startsWith("SELECT")) {
						...
						/*
						 * The actual number of columns retrieved has to be
						 * checked
						 */
						...
						while (display_rs.next()) {
							...
							if (colTypeName.equals("CHAR")) {
								colTypeName = colTypeName + "(" + Integer.toString(colWidth) + ")";
							} else if (colTypeName.equals("FLOAT")) {
								colTypeName += "(" + Integer.toString(colPrecision) + "," + Integer.toString(colScale) + ")";
							}
						...
						dbfDir = ft.getField(1);
						if (dbfDir.equals("OFF")) {
							try {
								spoolFileWriter.close();
							} catch (Exception spoolEx) {
								System.out.println("Unable to close spool file " + spoolEx.getMessage() + newLine);
							}
						} else {
							try {
								spoolFileWriter = new FileWriter(dbfDir);
								if (spoolFileWriter != null) {
									System.out.println("Output spooled to " + dbfDir);
								}
							} catch (Exception spoolEx) {
								System.out.println("Unable to spool to file " + spoolEx.getMessage() + newLine);
							}
						}
					} else if (cmdString.toUpperCase().startsWith("START ")) {
						ft = new FieldTokenizer(cmdString, ' ', false);
						dbfDir = ft.getField(1);
						if (!dbfDir.toUpperCase().endsWith(".SQL")) {
							dbfDir += ".SQL";
						}
						try {
							startReader = new BufferedReader(new FileReader(dbfDir));
						} catch (Exception ex) {
							startReader = null;
							throw new tinySQLException("No such file: " + dbfDir);
						}
						...
						try {
							...
							loadFileReader = new BufferedReader(new FileReader(dbfDir));
					...
					} else if (cmdString.toUpperCase().startsWith("SETSTRING") | cmdString.toUpperCase().startsWith("SETINT")) {
						...
						if (b2 > b1 & b1 > 0) {
							...
							if (true) {
								System.out.println("Set parameter[" + parameterIndex + "]=" + parameterString);
							}
	...
	private static Connection dbConnect(String dbfDir) throws SQLException {
		Connection con = null;
		DatabaseMetaData dbMeta;

		File conPath = new File(dbfDir);

		File[] fileList = conPath.listFiles();

		String tableName;
		ResultSet tables_rs;

		if (fileList == null) {
			System.out.println(dbfDir + " is not a valid directory.");
			return null;
		} else {
		...
		while (rs.next()) {
			...
			for (int ii = 0; ii < cols; ii++) {
				...
				if (first) {
					...
					if (true & meta.getColumnType(ii + 1) == Types.DATE) {
						testDate = rs.getDate(ii + 1);
						System.out.println("Value " + value + ", Date " + testDate.toString());
					}
			...
			try {
				...
				System.out.println("Exception writing to spool file " + writeEx.getMessage());
			}
		}
		return numCols;
	}
