...
public class MapController_03_Web extends GisComLib {
	...
	public final Projection setZoomLevel(int zoomLevelCurr ) {
		...
		if (true) {
			...
			zoomLevelCurr = zoomLevelCurr > zoomLevelMax ? zoomLevelMax
					: zoomLevelCurr;
		}
		...
		if (true) {
			...
			if (mbrNew != null) {
				...
				Projection projectionNew = Projection.getProject(mbrNew,
						pixelSize);
	...
	private double getLevelDist(double dist, final int n, final int N) {
		if (true) {
			if (n == N) {
				...
				double q = n
						* Math.sin((Math.PI / 2.0) * (n + 1.0) / (N + 1.0));
	...
	public MapController_03_Web(Projection projection, Mbr topLevelMbr,
			ZoomLevelList zoomLevelList) {
		super();
