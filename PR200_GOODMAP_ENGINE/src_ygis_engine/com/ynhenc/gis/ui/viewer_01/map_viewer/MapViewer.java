...
public class MapViewer extends JPanel implements Printable, DebugInterface {
	...
	public void whenComponentResized() {
		...
		if (size.width < 1 || size.height < 1) {
			// do nothing if width or height is less than 1
		} else {
	...
	public void layer_00_OpenMultipleFile() {
		...
		if (gisProject != null) {
			...
			if (projectFolder == null || !projectFolder.exists()) {
				...
				String message = "신규프로젝트를  먼저 생성하여야 합니다.";
				...
				if (projectFolder != null && projectFolder.exists()) {
					...
					if (fileList != null && fileList.length > 0) {
						...
						if (!fileFirst.getParentFile().equals(gisProject.getFolderMapLayer())) {
							// 레이어 폴더 밖의 파일을 선택했을 경우.
							Component comp = this;

							String message = "프로젝트 파일내의 SHP 파일을 선택하여 주세요!";
	...
	public void layer_02_OpenSingleUrl(Layer layer, int index, boolean needStyling, CoordinateConversion coordConv)
			throws Exception {

		if (false) {
			this.paint_07_Status(layer.getName() + "를 로딩중입니다!");
		}
	...
	public void paint_02_Layers(Graphics2D g2, Projection projection) {
		...
		if (gisProject != null) { // painting normal layer
			...
			painter.paint_01_GisProject(g2, gisProject, zoomLevelCurr, projection, req); // 레이어
			// 페인팅.

			painter.paint_03_Option(g2, gisProject, projection, zoomLevelCurr, then); // 옵션
			// 페인팅.

		} // end of painting normal layer

		if (false) { // paint selected shape layer
	...
	public void showAttribute(ShapeObject shape) {
		JOptionPane.showMessageDialog(this, shape.toString(), "속성", JOptionPane.INFORMATION_MESSAGE);
	}
	...
	public void paint(Graphics g) {
		...
		if (!this.isVisible() || !this.isShowing()) {
			// do not paint map image if this component is not visible or not
			// showing....
			return;
		}
		...
		Dimension size = this.getSize();
		if (size.width < 1 || size.height < 1) {
			// do not paint map if the size width or height is less than 1
			return;
		}
	...
	public void setGisProject(GisProject gisProject) {
		...
		if (true) {
			// 이전 프로젝트 크기와 오픈시 맵뷰어 크기가 다를 수가 있으므로.
	...
	// end of external ui components

	private String status;
	private JLabel statusLabel;
	private JTextField mbrBar;

	// selected shape layer
