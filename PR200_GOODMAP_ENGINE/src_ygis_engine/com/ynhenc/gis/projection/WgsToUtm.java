...
public class WgsToUtm extends CoordinateConversion {
	...
	public void setVariables( PntShort wgs ) {
		...
		this.rho = this.equatorialRadius
				* (1 - this.e * this.e)
				/ this.POW(1 - this.POW(this.e
						* this.SIN(laty), 2), 3 / 2.0);

		this.nu = this.equatorialRadius
				/ this.POW(1 - this.POW(this.e
						* this.SIN(laty), 2), (1 / 2.0));
		...
		this.S = this.A0 * laty - this.B0 * this.SIN(2 * laty) + this.C0
				* this.SIN(4 * laty) - this.D0 * this.SIN(6 * laty)
				+ this.E0 * this.SIN(8 * laty);

		this.K1 = this.S * this.k0;
		this.K2 = this.nu * this.SIN(laty) * this.COS(laty)
				* this.POW(this.sin1, 2) * this.k0 * (100000000) / 2;
		this.K3 = ((this.POW(this.sin1, 4) * this.nu * this.SIN(laty) * Math
				.pow(this.COS(laty), 3)) / 24)
				* (5 - this.POW(this.TAN(laty), 2) + 9 * this.e1sq
						* this.POW(this.COS(laty), 2) + 4
						* this.POW(this.e1sq, 2)
						* this.POW(this.COS(laty), 4))
				* this.k0
				* (10000000000000000L);

		this.K4 = this.nu * this.COS(laty) * this.sin1 * this.k0 * 10000;

		this.K5 = this.POW(this.sin1 * this.COS(laty), 3)
				* (this.nu / 6)
				* (1 - this.POW(this.TAN(laty), 2) + this.e1sq
						* this.POW(this.COS(laty), 2)) * this.k0
				* 1000000000000L;

		this.A6 = (this.POW(this.p * this.sin1, 6) * this.nu * this.SIN(laty)
				* this.POW(this.COS(laty), 5) / 720)
				* (61 - 58 * this.POW(this.TAN(laty), 2)
						+ this.POW(this.TAN(laty), 4) + 270 * this.e1sq
						* this.POW(this.COS(laty), 2) - 330 * this.e1sq
						* this.POW(this.SIN(laty), 2)) * this.k0 * (1E+24);
