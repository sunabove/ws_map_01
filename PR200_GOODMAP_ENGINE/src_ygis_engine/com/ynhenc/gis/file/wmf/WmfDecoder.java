...
public class WmfDecoder implements ImageProducer {
			...
			if (isConsumer(ic)) {
				...
				if (!err) {
					...
					for (int row = 0; row < height; row++) {
						ic.setPixels(0, row, width, 1, cmodel, rgbPixels, row
								* width, width);
					}
	...
	private void readWmf() throws IOException, InterruptedException {
		Dimension d = new Dimension(320, 240); // std window size, only if no
		// first header present
		Image offscreen;
		Graphics g;
	...
	private boolean readRecord(InputStream in) {
		...
		case META_SETPIXEL:
			...
			crco = new Color(getLoByteVal(params[0]), getHiByteVal(params[0]),
		...
		case META_POLYGON:
			...
			for (i = 0; i < params[0]; i++) {
				...
				if (debug) {
					System.out.println(Integer.toString(xpoints[i], 16) + " "
		...
		case META_POLYPOLYGON:
			...
			for (i = 0; i < params[0]; i++) {
				...
				if (debug) {
					System.out.println("Polygon #" + i + " Pts="
							+ params[i + 1]);
				}
		...
		case META_SELECTPALETTE:
			if (debug) {
				System.out.println("MetaSelectPalette:" + params[0] + " = "
						+ gdiObj[params[0]]);
			}
			if (gdiObj[params[0]].getMagic() == WmfDecObj.M_PALETTE) {
				...
				System.out
						.println(" ---- internal ERROR in MetaSelectPalette -----");
			}
			break;

		case META_SELECTCLIPREGION:
			if (debug) {
				System.out.println("MetaSelectClipRegion:" + params[0] + " = "
						+ gdiObj[params[0]]);
			}
			if (gdiObj[params[0]].getMagic() == WmfDecObj.M_CLIP) {
				DC.aktclip = gdiObj[params[0]];
				g
						.clipRect(DC.aktclip.getRect().x,
								DC.aktclip.getRect().y,
								DC.aktclip.getRect().width, DC.aktclip
										.getRect().height);
			} else {
				System.out
						.println(" ---- internal ERROR in MetaSelectClipregion -----");
			}
			break;

		case META_SELECTOBJECT:
			if (debug) {
				System.out.println("MetaSelectObject:" + params[0] + " = "
						+ gdiObj[params[0]]);
			}
			...
			case WmfDecObj.M_CLIP:
				...
				if (debug) {
					...
					g.drawRect(DC.aktclip.getRect().x, DC.aktclip.getRect().y,
							DC.aktclip.getRect().width,
							DC.aktclip.getRect().height);
				}
				g
						.clipRect(DC.aktclip.getRect().x,
								DC.aktclip.getRect().y,
		...
		case META_CREATEPENINDIRECT:
			...
			case PS_DASHDOT:
			case PS_DASHDOTDOT:
				System.out.println("MetaCreatePenIndirect: line attribute "
						+ params[0] + " ignored");
			case PS_INSIDEFRAME:
			case PS_SOLID:
				crco = new Color(getLoByteVal(params[3]),
						getHiByteVal(params[3]), getLoByteVal(params[4]));
				break;
			default:
				crco = Color.black;
				error = true;
			...
			if (!error) {
				add_handle(new WmfDecObj(PS_SOLID /* params[0] */, crco));
				if (debug) {
		...
		case META_CREATEBRUSHINDIRECT:
			if (debug) {
				System.out.println("MetaCreateBrushIndirect: Style_00_Object="
						+ params[0]);
			...
			case 1:
				...
				break;
			case 0:
				crco = new Color(getLoByteVal(params[1]), // BS_SOLID
				...
				break;
			case 2:
				crco = new Color(getLoByteVal(params[1]), // BS_HATCHED
						getHiByteVal(params[1]), getLoByteVal(params[2]));
				add_handle(new WmfDecObj((int) params[3], crco, DC.aktbackgnd,
						fr));
				if (debug) {
					System.out.println(crco);
				}
		...
		case META_CREATEREGION:
			...
			add_handle(new WmfDecObj(DC.xtransfer(params[7]), DC
					.ytransfer(params[8]), DC.xtransfer(params[9]), DC
					.xtransfer(params[10])));
		...
		case META_CREATEFONTINDIRECT:
			...
			if (debug) {
				System.out.println("MetaCreateFontIndirect: " + params[0] + " "
						+ params[1] + " " + s);
			}
		...
		case META_SETWINDOWEXT:
			...
			DC.winextX = params[1];
			if (debug) {
				System.out.println("MetaSetWindowExt:  X:" + DC.winextX
						+ "  Y:" + DC.winextY);
			}
			break;
		...
		case META_SETWINDOWORG:
			...
			DC.winorgX = params[1];
			if (debug) {
				System.out.println("MetaSetWindowOrg:  X:" + DC.winorgX
						+ "  Y:" + DC.winorgY);
			}
			break;
		...
		case META_SETTEXTCOLOR:
			DC.akttextc = new Color(getLoByteVal(params[0]),
					getHiByteVal(params[0]), getLoByteVal(params[1]));
			if (debug) {
		...
		case META_TEXTOUT:
			if (rdFunc == META_EXTTEXTOUT) {
				...
				d = params[3]; // option
				if (debug) {
					System.out.println("ExtTextOut:option ="
							+ Integer.toString(d, 16));
				}
			...
			if (DC.aktfont.getFontOrientation() != 0) {
				System.out
						.println("non horizontal text is not supported: " + s);
			} else {
			...
			if (debug) {
				System.out
						.println((rdFunc == META_EXTTEXTOUT ? "MetaExtTextOut: "
								: "MetaTextOut: ")
								+ (new String(text, 0))
								+ " (len="
								+ a
		...
		case META_SETTEXTJUSTIFICATION:
			...
			if (debug || params[0] != 0 || params[1] != 0) {
				System.out.println("MetaSetTextJustification: " + params[0]
						+ " " + params[1]);
			}
			break;
		...
		case META_SETBKMODE:
			if (debug) {
				System.out.println("MetaSetBkMode:"
						+ (params[0] == 1 ? "TRANSPARENT" : "OPAQUE"));
			}
		...
		case META_PATBLT:
			...
			if (debug) {
				System.out.println("MetaPatBlt: ROP code="
			...
			case DSTINVERT:
			default:
				System.out.println("unsupported ROP code:"
						+ Integer.toString(e, 16));
			}
		...
		case META_STRETCHBLT:
			...
			case SRCCOPY:
				...
				if (im != null) {
					...
					g.setColor(Color.black);
					g.drawLine(0, 0, DC.xtransfer(params[7]), DC
							.ytransfer(params[6]));
					g.drawLine(DC.xtransfer(params[7]), 0, 0, DC
							.ytransfer(params[6]));
				}
				break;
			default:
		...
		case META_DIBSTRETCHBLT:
			...
			case SRCCOPY:
				...
				if (im != null) {
					g.drawImage(im, d, c, b, a, fr);
					im = null;
				} else if (drawCross_if_error) { // draw a cross X
					g.setColor(Color.black);
		...
		case META_ESCAPE:
			...
			default:
				if (debug) {
					System.out.println("MetaEscape #" + params[0] + " "
							+ ((params[1] + 1) >>> 2) + " Words");
				}
			}
			break;
	...
	private void drawOpaqePattern(Graphics g, Image im, int x1, int y1, int x2,
			int y2, ImageObserver fr) {
	...
	private void showparams(short[] params, int recSize, int Func) {
		System.out.println("MetaRecord: " + Integer.toString(Func, 16)
				+ " RecSize=" + recSize);
	...
	private Image DIBBitmapImage(int off, short params[], Component comp) {
		...
		for (i = 0; i < colors; i++) { // palette data starts at params[off+20]
			...
			palette[i] += x;
			palette[i] -= 0x1000000; // add bit_pattern (unsigned)0xFF000000
			// (for alpha)
		}
		...
		case 1:
			for (k = height - 1, i = 0; k >= 0; k--) {
				for (l = 0; l < width; l += 16) {
					...
					for (j = 0, m = (m >> 8) | (m << 8); j + l < width
							&& j < 16; j++) {
						pixels[k * width + l + j] = (m & 0x8000) == 0 ? -0x1000000
								: -0x1;
						m <<= 1;
					}
					if (i % 2 != 0) {
		...
		Image im = comp.createImage(new MemoryImageSource(width, height,
				pixels, 0, width));
		pixels = null;
		return im;
	}
	...
	private Image OldBitmapImage(int off, short params[], Component comp) {
		...
		if ((params[off + 3] != 1) || (params[off + 4] != 1)) {
			System.out
					.println("sorry, the only supported format is: planes=1,bpp=1");
			return null;
		}
	...
	private final static int TA_UPDATECP = 1; 
...
class WmfDecObj {
	...
	Image createOpaqueImage(int hatchstyle, Color cc, Color back, Component fr) {
		...
		try {
			...
			MemoryImageSource mis = new MemoryImageSource(8, 8, ColorModel
					.getRGBdefault(), pixels, 0, 8);
